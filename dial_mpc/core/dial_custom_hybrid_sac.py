...
def rollout_us(step_env, state, us):
    """Rollout a sequence of controls and gather rewards and pipeline states.
    We remove values since we don't have them in SAC scenario."""
    def step(state, u):
...
class MBDPI:
    ...
    def reverse_once(self, state, rng, Ybar_i, noise_scale):
        ...
        # Evaluate MPPI weights based on rewards
        # Normalizing and weighting
        # We consider the last candidate (Ybar_i) as baseline
        ...
        # Weighted averages (not strictly needed beyond Ybar, but kept for logging)
    ...
    def reverse(self, state, YN, rng):
        ...
        with tqdm(range(self.args.Ndiffuse - 1, 0, -1), desc="Diffusing") as pbar:
            for i in pbar:
                ...
                freq = 1 / (time.time() - t0)
                pbar.set_postfix({"rew": f"{rews['rews'].mean():.2e}", "freq": f"{freq:.2f}"})
        return Yi
...
def main():
    ...
    # Load SAC policy and value parameters (if needed)
    # If you only want MPPI optimization (like PPO+MPPI), you can skip using SAC params directly.
    # If you want to incorporate SAC's policy, load them similarly as shown:
    ...
    env = UnitreeGo2EnvRL(
        ...
        get_value=False  # No value from env
    )
    ...
    with tqdm(range(Nstep), desc="Rollout") as pbar:
        for t in pbar:
            # forward single step using the MPPI solution
            state = step_env(state_init, Y0[0])
            rollout.append(state.pipeline_state)
            rews.append(state.reward)
            us.append(Y0[0])

            # update state_init for next iteration
            state_init = state

            # update Y0 by shifting
            Y0 = mbdpi.shift(Y0)
