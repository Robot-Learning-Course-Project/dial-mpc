...
def rollout_us(step_env, state, us):
    def step(state, u):
        ...
        return state, (state.reward, state.info["value"], state.pipeline_state)

    _, (rews, values, pipline_states) = jax.lax.scan(step, state, us)
    return rews, values, pipline_states
...
class MBDPI:
    ...
    def reverse_once(self, state, rng, Ybar_i, noise_scale):
        ...
        # 3. replace last k steps of cost with value
        # rewss = rewss.at[:, -32:].set(valuess[:, -32:])    
        # rewss = rewss.at[:].set(valuess[:])    
        ...
        # REMOVE: jax.debug.print("any rew is nan: {y}", y=jnp.any(jnp.isnan(rews)))
...
def main():
    ...
    art.tprint("LeCAR @ CMU\nPPO-DIALMPC", font="big", chr_ignore=True)
    ...
    with tqdm(range(Nstep), desc="Rollout") as pbar:
        for t in pbar:
            ...
            # REMOVE:  print(f"action taken: {np.array(Y0[0])}")
